const teams = [
  ...
  { name: "Liqwid", url: "https://www.liqwid.finance/", logo: "liqwid.svg" },
  {
    ...
    logo: "midgard.png",
    linkLabel: "Ver la fuente pública de apoyo de Midgard Labs",
  },
...
const supporters = [
  ...
  ["Votante directo · …wj36md", "drep1yt4vlxgkllldxtwrmzj2ymwnc5qw0cmzvkn4nxpxkpgt4fgwj36md", 4182312.1868, 90],
  ["Votante directo · …lah2jj", "drep1y2r5kww2gj4j6l2h7065fk5jtfhzxwf37ryygkdx4qhmpvqlah2jj", 4013963.627915, 93],
  ...
  ["Votante directo · …0sduqg", "drep1ygw6c6ljek5re3dq4vmtpneqqlvccvcgwc8l7gjljqdfa0g0sduqg", 2405146.430267, 110],
  ...
  ["Votante directo · …zgtg0g", "drep1yt05vsgtruppuej5jzmswd7tj86ujanydq27qxc2nksrt8szgtg0g", 1310598.960277, 140],
  ...
  ["Votante directo · …pavmcg", "drep1yf7nwlcvdql5kqvaqvmzyj5azqcc5s4y4wm2sm0ng3nzkkgpavmcg", 182165.669375, 258],
  ...
  ["Votante directo · …94cxus", "drep1ytghtnjnet6dyhfgsucu9fyqhdxmph2z3e967ufduh9ypwq94cxus", 107152.279491, 283],
  ["World Peace", "drep1yfgssww5qn4w5w9g2pmtj94rg0l2jtcz87w9d4m459dpqhqm9g4pu", 92547.737955, 288],
  ["Votante directo · …xvnagh", "drep1y2x9n3y7mj92klgq6klnkrvwdawhwcsl8kfc3sctff5a86cxvnagh", 15922.8029, 335],
...
async function refreshLiveVoteStatus() {
  ...
  try {
    ...
    const sourceName = ["AdaStat", "GovTool", "Koios"].includes(status.source) ? status.source : "datos públicos en cadena";
    ...
    const voterDetail = hasValidYesVotes ? ` ${yesVotes} DReps votan Sí actualmente.` : "";
    const accessibleStatus = `${yesPercent.toFixed(2)}% del poder de voto elegible de los DReps es Sí.${voterDetail} En vivo desde ${sourceName}, actualizado ${refreshedLabel}.`;

    lastLiveVoteStatus = { yesPercent, yesVotes: hasValidYesVotes ? yesVotes : null, accessibleStatus, sourceName };

    liveVoteEyebrow.textContent = `EN VIVO · ${sourceLabel}`;
    liveYesPercent.textContent = `${yesPercent.toFixed(2)}%`;
    liveVoteLabel.textContent = "PODER SÍ DE DREPS";
    liveVoteCore.dataset.feedState = "live";
    liveVoteCore.setAttribute("aria-label", `${accessibleStatus} Abre el rastreador en vivo de AdaStat.`);
    ...
    if (lastLiveVoteStatus) {
      liveVoteEyebrow.textContent = `${lastLiveVoteStatus.sourceName.toUpperCase()} · ÚLTIMO EN VIVO`;
      liveVoteLabel.textContent = "RECONECTANDO";
      liveVoteCore.dataset.feedState = "stale";
      liveVoteCore.setAttribute("aria-label", `${lastLiveVoteStatus.accessibleStatus} La actualización en vivo no está disponible temporalmente. Abre el rastreador de AdaStat.`);
      liveVoteCore.setAttribute("title", "Mostrando el último valor en vivo mientras se reconecta con AdaStat");
      if (liveVoteAnnouncement && lastAnnouncedVoteKey !== `stale:${lastLiveVoteStatus.yesPercent}`) {
        liveVoteAnnouncement.textContent = `La actualización en vivo de ${lastLiveVoteStatus.sourceName} no está disponible temporalmente. Mostrando el último valor correcto.`;
        lastAnnouncedVoteKey = `stale:${lastLiveVoteStatus.yesPercent}`;
      }
    } else {
      liveVoteEyebrow.textContent = "ADASTAT";
      liveYesPercent.textContent = "--.--%";
      liveVoteLabel.textContent = "FUENTE NO DISPONIBLE";
      liveVoteCore.dataset.feedState = "error";
      liveVoteCore.setAttribute("aria-label", "La fuente en vivo del poder de voto Sí de los DReps no está disponible temporalmente. Abre el rastreador de AdaStat.");
      liveVoteCore.setAttribute("title", "Fuente de votos en vivo no disponible temporalmente");
      if (liveVoteAnnouncement && lastAnnouncedVoteKey !== "error") {
        liveVoteAnnouncement.textContent = "La fuente de votos en vivo de AdaStat no está disponible temporalmente.";
...
logoField.innerHTML = `
  <div class="field-core" aria-hidden="true">
    <span class="field-core__label"><span>CARDANO</span><strong>DeFi</strong><span>RED PRIME</span></span>
  </div>
` + teams.map((team, index) => {
  ...
  <a
    ...
    aria-label="${team.linkLabel || `Visitar ${team.name}`}"
  >
...
function getPowerTier(power) {
  ...
  if (power >= 25_000_000) return ["major", "MAYOR"];
  if (power >= 5_000_000) return ["established", "CONSOLIDADO"];
  if (power >= 1_000_000) return ["growing", "EMERGENTE"];
  return ["community", "COMUNIDAD"];
}
...
function getSupporterInitials(name) {
  if (name.startsWith("Votante directo")) return name.slice(-2).toUpperCase();
  const words = name
    .replace(/\([^)]*\)/g, "")
    .replace(/[^\p{L}\p{N}$]+/gu, " ")
    .trim()
...
function renderSupporters(items) {
  drepGrid.innerHTML = items.map(([name, id, power, rank]) => {
    ...
    const profileLabel = profile ? (profile.kind === "logo" ? "ORGANIZACIÓN VERIFICADA" : "PERFIL PÚBLICO VERIFICADO") : "DREP EN CADENA";
    ...
    <a
      ...
      rel="noreferrer"
      title="${id}"
      aria-label="${name}, ${formatAccessiblePower(power)} ADA de poder de voto, voto Sí verificado, abrir el registro del DRep"
    >
      <span class="drep-card__top">
        <span class="drep-order">#${String(order).padStart(2, "0")} · PUESTO ${rank}</span>
      ...
      <span class="drep-card__bottom">
        <span class="drep-power"><b>${formatPower(power)}</b><small>ADA de poder de voto</small></span>
        <span class="verified-mark">SÍ ✓</span>
      </span>
    </a>
  `;
  }).join("");
