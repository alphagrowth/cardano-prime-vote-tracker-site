const teams = [
  ...
  { name: "Liqwid", url: "https://www.liqwid.finance/", logo: "liqwid.svg" },
  {
    ...
    logo: "midgard.png",
    linkLabel: "Midgard Labsの公開の支持元を見る",
  },
...
const supporters = [
  ...
  ["直接投票者 · …wj36md", "drep1yt4vlxgkllldxtwrmzj2ymwnc5qw0cmzvkn4nxpxkpgt4fgwj36md", 4182312.1868, 90],
  ["直接投票者 · …lah2jj", "drep1y2r5kww2gj4j6l2h7065fk5jtfhzxwf37ryygkdx4qhmpvqlah2jj", 4013963.627915, 93],
  ...
  ["直接投票者 · …0sduqg", "drep1ygw6c6ljek5re3dq4vmtpneqqlvccvcgwc8l7gjljqdfa0g0sduqg", 2405146.430267, 110],
  ...
  ["直接投票者 · …zgtg0g", "drep1yt05vsgtruppuej5jzmswd7tj86ujanydq27qxc2nksrt8szgtg0g", 1310598.960277, 140],
  ...
  ["直接投票者 · …pavmcg", "drep1yf7nwlcvdql5kqvaqvmzyj5azqcc5s4y4wm2sm0ng3nzkkgpavmcg", 182165.669375, 258],
  ...
  ["直接投票者 · …94cxus", "drep1ytghtnjnet6dyhfgsucu9fyqhdxmph2z3e967ufduh9ypwq94cxus", 107152.279491, 283],
  ["World Peace", "drep1yfgssww5qn4w5w9g2pmtj94rg0l2jtcz87w9d4m459dpqhqm9g4pu", 92547.737955, 288],
  ["直接投票者 · …xvnagh", "drep1y2x9n3y7mj92klgq6klnkrvwdawhwcsl8kfc3sctff5a86cxvnagh", 15922.8029, 335],
...
async function refreshLiveVoteStatus() {
  ...
  try {
    ...
    const sourceName = ["AdaStat", "GovTool", "Koios"].includes(status.source) ? status.source : "公開オンチェーンデータ";
    ...
    const voterDetail = hasValidYesVotes ? ` 現在${yesVotes}のDRepが賛成票を投じています。` : "";
    const accessibleStatus = `対象となるDRep投票力の${yesPercent.toFixed(2)}%が賛成です。${voterDetail} ${sourceName}からライブ配信、更新: ${refreshedLabel}。`;

    lastLiveVoteStatus = { yesPercent, yesVotes: hasValidYesVotes ? yesVotes : null, accessibleStatus, sourceName };

    liveVoteEyebrow.textContent = `ライブ · ${sourceLabel}`;
    liveYesPercent.textContent = `${yesPercent.toFixed(2)}%`;
    liveVoteLabel.textContent = "DREP賛成投票力";
    liveVoteCore.dataset.feedState = "live";
    liveVoteCore.setAttribute("aria-label", `${accessibleStatus} AdaStatのライブトラッカーを開く。`);
    ...
    if (lastLiveVoteStatus) {
      liveVoteEyebrow.textContent = `${lastLiveVoteStatus.sourceName.toUpperCase()} · 最終ライブ`;
      liveVoteLabel.textContent = "再接続中";
      liveVoteCore.dataset.feedState = "stale";
      liveVoteCore.setAttribute("aria-label", `${lastLiveVoteStatus.accessibleStatus} ライブ更新は一時的に利用できません。AdaStatのトラッカーを開いてください。`);
      liveVoteCore.setAttribute("title", "AdaStatへ再接続する間、最後のライブ値を表示しています");
      if (liveVoteAnnouncement && lastAnnouncedVoteKey !== `stale:${lastLiveVoteStatus.yesPercent}`) {
        liveVoteAnnouncement.textContent = `${lastLiveVoteStatus.sourceName}のライブ更新は一時的に利用できません。最後に取得できた値を表示しています。`;
        lastAnnouncedVoteKey = `stale:${lastLiveVoteStatus.yesPercent}`;
      }
    } else {
      liveVoteEyebrow.textContent = "ADASTAT";
      liveYesPercent.textContent = "--.--%";
      liveVoteLabel.textContent = "フィード利用不可";
      liveVoteCore.dataset.feedState = "error";
      liveVoteCore.setAttribute("aria-label", "DRepの賛成投票力のライブフィードは一時的に利用できません。AdaStatのトラッカーを開いてください。");
      liveVoteCore.setAttribute("title", "ライブ投票フィードは一時的に利用できません");
      if (liveVoteAnnouncement && lastAnnouncedVoteKey !== "error") {
        liveVoteAnnouncement.textContent = "AdaStatのライブ投票フィードは一時的に利用できません。";
...
logoField.innerHTML = `
  <div class="field-core" aria-hidden="true">
    <span class="field-core__label"><span>CARDANO</span><strong>DeFi</strong><span>PRIMEネットワーク</span></span>
  </div>
` + teams.map((team, index) => {
  ...
  <a
    ...
    aria-label="${team.linkLabel || `${team.name}のサイトを開く`}"
  >
...
function getPowerTier(power) {
  if (power >= 100_000_000) return ["keystone", "最上位"];
  if (power >= 25_000_000) return ["major", "主要"];
  if (power >= 5_000_000) return ["established", "中堅"];
  if (power >= 1_000_000) return ["growing", "成長中"];
  return ["community", "コミュニティ"];
}
...
function getSupporterInitials(name) {
  if (name.startsWith("直接投票者")) return name.slice(-2).toUpperCase();
  const words = name
    .replace(/\([^)]*\)/g, "")
    .replace(/[^\p{L}\p{N}$]+/gu, " ")
    .trim()
...
function renderSupporters(items) {
  drepGrid.innerHTML = items.map(([name, id, power, rank]) => {
    ...
    const profileLabel = profile ? (profile.kind === "logo" ? "検証済み組織" : "検証済み公開プロフィール") : "オンチェーンDRep";
    ...
    <a
      ...
      rel="noreferrer"
      title="${id}"
      aria-label="${name}、投票力${formatAccessiblePower(power)} ADA、賛成票を検証済み、DRepの記録を開く"
    >
      <span class="drep-card__top">
        <span class="drep-order">#${String(order).padStart(2, "0")} · 順位 ${rank}</span>
      ...
      <span class="drep-card__bottom">
        <span class="drep-power"><b>${formatPower(power)}</b><small>ADAの投票力</small></span>
        <span class="verified-mark">賛成 ✓</span>
      </span>
    </a>
  `;
  }).join("");
  setCountTarget(supporterCount, supporters.length);
  drepResultCount.textContent = `検証済みの賛成投票者 ${supporters.length}件中 ${items.length}件を表示`;
